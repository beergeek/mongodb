...
plan mongodb::mongod_linux (
  ...
  # Configure database
  run_task('mongodb::mongod_server_config', $nodes, x509_path => $x509_path, ca_path => $ca_path, mongodb_service_user => $mongodb_service_user, config_file => $config_file, db_path => $db_path, log_file => $log_file, base_path => $base_path, repset => $repset, keyfile => $keyfile, keyfile_path => $keyfile_path, bindnic => $bindnic, port => $port, extra_config => $extra_config, sharding => $sharding, '_run_as' => 'root')
  # Run service
  run_task('mongodb::mongod_server_service', $nodes, config_file => $config_file, '_run_as' => $mongodb_service_user )
  # Initialise Replica Set
  run_task('mongodb::mongod_rs_initiate', $primary, nodes => $rs_nodes, port => $port, repset => $repset, x509_path => $x509_path, ca_path => $ca_path)
  # Sleep for a bit to ensure stuff is setup
  ctrl::sleep(30)
  # Create admin user
  run_task('mongodb::mongod_admin_user', $primary, user => $admin_user, passwd => $admin_password, port => $port, x509_path => $x509_path, ca_path => $ca_path, '_run_as' => $mongodb_service_user )
}
